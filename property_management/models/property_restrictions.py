...
class PropertyRestrictions(models.Model):
    _description = "Etiquetas Restricciones"
    _name = 'property.restrictions'
    _inherit = ["mail.thread.cc", "mail.activity.mixin", "utm.mixin"]

    name = fields.Char(_('Nombre'), index=True, required=True)
    
    _sql_constraints = [('name_uniq', 'unique (name)', _("La Restricción ingresada ya se encuentra registrada en el sistema."))]
    ...
    def fields_get(self, fields=None):
        ...
        res = super(PropertyRestrictions, self).fields_get()
