...
class PropertyManagementCrmLead(models.Model):
    ...
    property_url = fields.Char(
        string='Url',
        compute='_compute_property_url',
        inverse='_inverse_property_url',
        store=True,
        website_form_blacklisted=False
    )

    def _compute_property_url(self):
        self.property_url = False

    @api.depends('property_url')
    def _inverse_property_url(self):
        _logger.info('here')
        for record in self:
            if record.property_url:
                _logger.info(record.property_url)
                pid = int(record.property_url.split('-')[-1])
