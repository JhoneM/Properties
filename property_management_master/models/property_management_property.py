...
class PropertyManagementProperty(models.Model):
    ...
    city = fields.Char(string=_("Comuna"))
    ...
    community_name = fields.Char(string=_("Nombre comunidad"))
    ...
    image_ids = fields.One2many('property.management.property.image', 'property_parent_id', 'Imagenes de la propiedad')
    ...
    attachments = fields.One2many('property.attachments','property_id', string='Adjuntos')
    ...
    sign = fields.Boolean(string=_("Firma"))
    agreed_commission_currency = fields.Many2one('res.currency', string=_("Moneda"), domain=_get_currency, default=_default_currency)
    agreed_commission_fixed = fields.Float(string=_("Precio fijo"), digits=(16, 3))
    agreed_commission_percent = fields.Float(string=_("Porcentaje"), digits=(16, 3))
    compute_price = fields.Selection([
        ('fixed', 'Precio fijo'),
        ('percentage', 'Porcentaje'),], string=_("Comisión Pactada"), index=True, default='fixed', required=True)
    ...
    pivot_evaluation = fields.Selection([('0', 'Cero'), ('1', 'Primero'), ('2', 'Segundo'),('3', 'Tercero')])
            ...
            if record.cons_year:
                if record.cons_year.isdigit():
                    ...
                    if val_year >= 1900.0 and val_year <= 2100.0:
                        ...
                        raise exceptions.ValidationError(_("Año de construcción: No esta en un rango valido"))        
                else:
...
class PropertyManagementOrigin(models.Model):
    ...
    def _onchange_name(self):
        if self.no_editable:
            raise exceptions.ValidationError(_("No puede modificar este valor.")) 
